...
if (plot3d) :
    ...
    ax.set_zlabel(r'$E$')
    ax.view_init(20,30)
    plt.title('E vs. (kx,ky) for haldane')
    plt.savefig('img/Ham_haldane3d.png',dpi=300)
    plt.close()
else:
    for i in range(0,N1,4):
        ...
        plt.xlabel('ky')
        plt.ylabel('E')
        plt.title('E vs. ky for haldane at kx={:.2f}'.format(kx))
        plt.savefig('img/Ham_haldane2d/'+str(i)+'.png', dpi=300)
        plt.close()
