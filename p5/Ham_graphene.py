...
for j in range(N1):
    for k in range(N2):
        ...
        eigs=np.linalg.eigh(HK[i:i+2, i:i+2])[0]
        yarr1.append(eigs[0])
        yarr2.append(eigs[1])
...
if (plot3d) :
    ...
    ax.set_zlabel(r'$E$')
    ax.view_init(20,30)
    plt.title('E vs. (kx,ky) for graphene')
    plt.savefig('img/Ham_graphene3d.png',dpi=300)
    plt.close()
else:
    for i in range(N1):
        kx=xarr[i*N2][0]
        ...
        plt.xlabel('ky')
        plt.ylabel('E')
        plt.title('E vs. ky for graphene at kx={:.2f}'.format(kx))
        plt.savefig('img/Ham_graphene2d/'+str(i)+'.png', dpi=300)
        plt.close()
